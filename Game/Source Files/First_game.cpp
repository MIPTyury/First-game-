...
int main() {
    ...
    int modeHeight = 800;
    int modeWidth = 600;
    sf::RenderWindow window(sf::VideoMode(modeWidth, modeHeight), "Bricks ball breaker", sf::Style::Default, settings);
    ...
    ball_texture.loadFromFile("../images/ball_image.png");
    ball_texture.setSmooth(true);

    sf::Sprite ball_sprite;
    ball_sprite.setTexture(ball_texture);
    ball_sprite.setPosition(290, 660);
    sf::Color ball_color(168, 168, 168);
    ball_sprite.setColor(ball_color);
    ...
    V.x = truncf((2 * alpha) / sqrtf(alpha * alpha + 1));
    V.y = truncf(2 / sqrtf(alpha * alpha + 1));
    ...
    while (window.isOpen()) {
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 6; j++) {
                block_collision(row[i][j], V, ball_sprite, ball_hit_sound, ball_radius);
            }
        }
        ...
        if (ball_sprite.getPosition().x >= modeWidth - 2 * ball_radius ||
            (ball_sprite.getPosition().x + 2 * ball_radius == platform.getPosition().x &&
             (ball_sprite.getPosition().y >= platform.getPosition().y &&
        ...
        if (ball_sprite.getPosition().y <= 0 ||
            (ball_sprite.getPosition().y + 2 * ball_radius == platform.getPosition().y &&
             (ball_sprite.getPosition().x + 2 * ball_radius >= platform.getPosition().x &&
              ball_sprite.getPosition().x < platform.getPosition().x + plat_size.x))) {
            V.y = -V.y;
            ball_hit_sound.play();
        }
        ...
        if (ball_sprite.getPosition().y >= modeHeight - 2 * ball_radius) {
